...
// Util
import { readFileAsync, hasDuplicates, DEFAULT_BED_DATA } from "../../utils";
    ...
    readFileAsync(file)
      .then((result) => {
        ...
        try {
          ...
          res = getCleanedAndValidated(raw);
        } catch (err) {
 ...
 */
const getCleanedAndValidated = (rawData) => {
  const cleanData = {};
  const errors = [];
  ...
  if (_gridData.length === 0) {
    ...
    _gridData.forEach((_bedspace, _bedspace_index, _object) => {
      ...
      if (!keysMatchDefault) {
        ...
        _gridData[_bedspace_index] = { ...DEFAULT_BED_DATA, ..._bedspace };
      }
      ...
      */
      ...
      shouldBeString.forEach((_s) => {
        _gridData[_bedspace_index][_s] = checkForStringTypeAndConvert(
          _bedspace,
          _s
        );
      });
      ...
      _bedspace_contingencies.forEach((_cv, _ci) => {
        if (typeof _cv !== "string") {
          ...
          _gridData[_bedspace_index]["contingencies"][_ci] = _cv + "";
        }
      });
      ...
      _bedspace_nestedContent.forEach((_nv, _ni) => {
        if (typeof _nv !== "object") {
          ...
          _gridData[_bedspace_index]["nestedContent"].splice(_ni, 1);
        }
        ...
        if (_nestedContent_items?.length !== 0) {
          _nestedContent_items.forEach((_iv, _ii) => {
            if (typeof _iv !== "object") {
              ...
              // remove this element from the nestedContent array
              _gridData[_bedspace_index]["nestedContent"][_ni]["items"].splice(
                _ii,
                1
              );
            }
